...
With `auto_start_runs`, the existing orchestrator starts one `work_on_ticket` run for an immediately assigned implementer. The existing run-start gate owns `Ready → In Progress`. If the assignee already has an active consultation, assignment in `Ready` is the durable deferred-work marker: the response's terminal hook starts the ownership run idempotently after releasing the active-run guard, but only while no manual recommendation is pending and the retained assignee is still an enabled, non-Tech-Lead implementer. Successful refinement consultations are not auto-dispatched, so a valid handoff produces exactly one implementer run and a manual or invalid handoff produces no run.

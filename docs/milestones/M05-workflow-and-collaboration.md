...
The Tech Lead completion itself leaves the ticket in `Ready`. A valid target follows `workflow.auto_assign` for the Ready status: immediate assignment when enabled, otherwise a pending recommendation for human approval. With `auto_start_runs`, an immediate assignment queues exactly one implementer `work_on_ticket` run; the existing run-start gate owns `Ready → In Progress`. If that implementer is already answering a consultation, the Ready assignment durably defers its ownership run until the active response terminates, and the active-run guard keeps the later start idempotent. The terminal response only releases deferred ownership when no manual recommendation is pending and the retained assignee is still an enabled, non-Tech-Lead implementer. Missing, blank, unknown, disabled, or non-implementer targets leave status and assignment unchanged, start nobody, and add an actionable system comment listing enabled implementer keys.
